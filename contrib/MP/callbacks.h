...
//    int const threadNo = context.getIntInfo(IloCplex::Callback::Context::Info::ThreadId);
    ...
    for (uint64_t sp_id = 0; sp_id < shared_info_.num_subproblems; ++sp_id) {
        ...
        assert(shared_info_.dual_values[sp_id].getSize() == master_variables_.getSize());
        ...
        if (violation > _violation_threshold) {
            ...
            expr -= IloScalProd(shared_info_.dual_values[sp_id], shared_info_.master_variables_value);
